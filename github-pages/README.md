# Layang GitHub Pages

This folder is the public landing-site source for Layang.

## Pages

- `index.html` - product landing page for Layang 1.1.2, including REST, WebSocket, gRPC, gRPC-Web, local mocks, gateway access, certificate settings, documentation, workspace files, CLI, and download sections.

## Assets

- `assets/layang-logo.png`
- `assets/layang-app-screenshot.png`
- `assets/layang-app-ws.png`
- `assets/layang-mock-stream.png`
- `assets/layang-app-documentation.png`
- `assets/styles.css`

## Local checks

GitHub Pages deploys this directory directly from `.github/workflows/pages.yaml`. For local preview, serve `github-pages/` with any static HTTP server.

## Publish

The repository workflow at `.github/workflows/pages.yaml` publishes this folder directly to GitHub Pages. You can also copy the folder contents into a `gh-pages` branch manually.

## Custom domain

- `CNAME` is configured for `layang.mff.web.id`.
- `index.html`, `robots.txt`, and `sitemap.xml` use the same canonical production URL for search indexing.
