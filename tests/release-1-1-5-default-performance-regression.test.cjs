...
test('release metadata is consistently 1.1.5', () => {
  const pkg = JSON.parse(read('package.json'));
  assert.equal(pkg.version, '1.1.5');

  const readme = read('README.md');
  assert.match(readme, /version-1\.1\.5-blue/);
  assert.match(readme, /## Release 1\.1\.5/);

  const landing = read('github-pages/index.html');
  assert.match(landing, /"softwareVersion": "1\.1\.5"/);
  assert.match(landing, /Layang v1\.1\.5 is officially released/);

  const changelog = read('CHANGELOG.md');
  assert.match(changelog, /^## 1\.1\.5\b/m);
});
...
test('desktop development renderer defaults to port 12999', () => {
  const pkg = JSON.parse(read('package.json'));
  assert.match(pkg.scripts.desktop, /--port 12999/);
  assert.doesNotMatch(pkg.scripts.desktop, /run dev -- --port/);
  assert.match(pkg.scripts.desktop, /localhost:12999\/playground/);

  const createWindow = read('electron/window/create-window.cjs');
  assert.match(createWindow, /http:\/\/localhost:12999\/playground/);
});
