...
async function migrateGitWorkspace(rootDirectory, options = {}) {
  ...
  const result = {
    ...
    mode,
    currentVersion,
    targetVersion: WORKSPACE_VERSION,
    needsMigration,
...
async function writeGitWorkspace(rootDirectory, bundle) {
  ...
  if (previousManifest && Number(previousManifest.version || 1) < WORKSPACE_VERSION) {
    await backupWorkspaceBeforeMigration(root, Number(previousManifest.version || 1));
  }

  // Proto revisions are immutable. Validate and persist them before any other workspace write
  // so an external edit cannot leave a partially updated workspace.
  await writeProtoLibraries(root, arrayOr(project.protoLibraries));
  ...
  return { root, version: WORKSPACE_VERSION };
}
...
async function writeMocks(root, project) {
  ...
  if (!Object.keys(methodFiles).length && typeof grpc.scenarioText === "string" && grpc.scenarioText.trim()) {
    const parsed = parseScenarioBundle({ format: grpc.format, scenarioText: grpc.scenarioText });
    const grouped = groupGrpcScenarios(parsed.scenarios);
...
async function backupWorkspaceBeforeMigration(root, fromVersion) {
  const backupRoot = path.join(root, LOCAL_DIR, "backups", `workspace-v${fromVersion}-before-v${WORKSPACE_VERSION}`);
  ...
  await writeTextAtomic(
    ...
    stringifyYaml({
      ...
      backup: {
        ...
        targetWorkspaceVersion: WORKSPACE_VERSION,
