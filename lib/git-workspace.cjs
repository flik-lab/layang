    ...
    for (const serviceMatch of text.matchAll(/service\s+([A-Za-z_][\w]*)\s*\{([\s\S]*?)\}/g)) {
      ...
      for (const methodMatch of serviceMatch[2].matchAll(
        ...
        const candidate = {
          ...
          // Match protobufjs' resolved type names used by the renderer. Keeping
          // short names here (for example `HelloRequest`) caused the desktop
          // workspace loader to overwrite every saved binding with a different
          // signature from `demo.v1.HelloRequest` on each `pnpm desktop` start.
          requestType: canonicalProtoTypeName(methodMatch[3], packageName),
          responseType: canonicalProtoTypeName(methodMatch[5], packageName),
...
function canonicalProtoTypeName(value, packageName) {
  const typeName = String(value || "").trim();
  if (!typeName) return "";
  if (typeName.startsWith(".")) return typeName.slice(1);
  if (!packageName || typeName === packageName || typeName.startsWith(`${packageName}.`)) return typeName;
  if (typeName.startsWith("google.protobuf.")) return typeName;
