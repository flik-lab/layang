...
function getStaleFileRuntimeUpdate(active, payload, source, now = Date.now(), quietPeriodMs = 3000) {
  if (source !== "file") return { stale: false, reason: "not-file", retryAfterMs: 0 };
  if (!active?.hasUiRuntimeOverride || !active?.lastUiRuntimeUpdateAt) {
    return { stale: false, reason: "no-ui-override", retryAfterMs: 0 };
  }

  const lastUiRuntimeUpdateAt = Number(active.lastUiRuntimeUpdateAt || 0);
  const elapsedMs = now - lastUiRuntimeUpdateAt;
  const withinUiQuietPeriod = elapsedMs < quietPeriodMs;
  if (withinUiQuietPeriod) {
    return {
      stale: true,
      reason: "ui-quiet-period",
      retryAfterMs: Math.max(25, quietPeriodMs - elapsedMs + 25),
    };
  }

  const fileRuntimeMtimeMs = getFileRuntimeMtimeMs(payload);
  if (fileRuntimeMtimeMs <= 0) return { stale: true, reason: "missing-file-mtime", retryAfterMs: 0 };
  if (fileRuntimeMtimeMs < lastUiRuntimeUpdateAt) {
    return { stale: true, reason: "stale-file-mtime", retryAfterMs: 0 };
  }

  return { stale: false, reason: "fresh-file-mtime", retryAfterMs: 0 };
}

function isStaleFileRuntimeUpdate(active, payload, source, now = Date.now(), quietPeriodMs = 3000) {
  return getStaleFileRuntimeUpdate(active, payload, source, now, quietPeriodMs).stale;
}
...
function shouldIgnoreFileRuntimeUpdate(active, runtime, payload, source, now = Date.now(), quietPeriodMs = 3000) {
  ...
  const staleFileUpdate = getStaleFileRuntimeUpdate(active, payload, source, now, quietPeriodMs);
  if (staleFileUpdate.stale) {
    return {
      ignore: true,
      reason: staleFileUpdate.reason,
      retryAfterMs: staleFileUpdate.retryAfterMs,
    };
  }
