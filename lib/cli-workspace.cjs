...
async function readWorkspace(workspaceDirectory) {
  ...
  const [requestTabs, requestFiles, environments, mockServer, protoFiles, splitMock] = await Promise.all([
...
function discoverRunItems(workspace, options) {
  ...
  const fallbackItem = selectedMethodKey
    ...
    : [];
  const candidates = tabs.length ? tabs : fallbackItem;
  const methodFilter = options.method || "";
  return candidates
    .filter((item) => !methodFilter || item.methodKey === methodFilter || item.methodKey.endsWith(`/${methodFilter}`))
    .map((item, index) => ({
      id: item.id || `request-${index + 1}`,
      title: item.title || item.methodKey || `Request ${index + 1}`,
      methodKey: item.methodKey || selectedMethodKey,
      requestJson: item.requestJson || "{}",
      metadata: Array.isArray(item.metadata) ? item.metadata : [],
      transportMode: options.transport || item.transportMode || project.transportMode || "native-grpc",
      target:
        options.target ||
        resolveTarget(
          {
...
function validateWorkspace(workspace) {
  ...
  if (!Array.isArray(project.protoFiles) || project.protoFiles.length === 0)
    errors.push("No proto files found. Add files under protos/ or import proto files in the app.");
  if (!Array.isArray(project.requestTabs) || project.requestTabs.length === 0)
    warnings.push("No saved request tabs found. The CLI will use selectedMethodKey/requestJson if available.");
  if (!project.selectedMethodKey && (!project.requestTabs || project.requestTabs.length === 0))
    errors.push("No selected method or request tabs found to run.");
