...
async function deleteSchema(root, selector) {
  ...
  project.methodDocs = markMissingBindings(project.methodDocs, library.id);
  project.docResults = markMissingBindings(project.docResults, library.id);
  if (project.mockServer?.methodBindings && typeof project.mockServer.methodBindings === "object") {
    project.mockServer.methodBindings = Object.fromEntries(
      Object.entries(project.mockServer.methodBindings).map(([key, binding]) => [
        key,
        binding?.libraryId === library.id ? { ...binding, status: "library-missing" } : binding,
      ]),
    );
  }
...
function findVersion(library, selector) {
  const query = String(selector || "").trim().toLowerCase();
  return (
    (Array.isArray(library?.versions) ? library.versions : []).find(
      (version) =>
        String(version?.id || "").toLowerCase() === query ||
        String(version?.version || "").toLowerCase() === query,
    ) || null
  );
}

function pickNextActiveVersion(library, excludedVersionId = "") {
  ...
  return (
    versions.find(
      (version) =>
        ...
        version.id === library.defaultVersionId,
    ) ||
    versions.find((version) => version.id !== excludedVersionId && version.lifecycle !== "archived") ||
    versions.find((version) => version.id !== excludedVersionId) ||
    null
  );
}

async function archiveSchema(root, selector) {
  const git = await requireGitWorkspace(root);
  const project = structuredClone(git.project);
  const library = findLibrary(project, selector);
  if (!library) throw new Error(`Schema ${selector} was not found.`);
  if (library.lifecycle === "archived") return { action: "already-archived", library };
  const now = new Date().toISOString();
  library.lifecycle = "archived";
  library.archivedAt = now;
  library.updatedAt = now;
  if (project.activeProtoLibraryId === library.id) {
    const nextLibrary = (project.protoLibraries || []).find(
      (candidate) => candidate.id !== library.id && candidate.lifecycle !== "archived",
    );
    project.activeProtoLibraryId = nextLibrary?.id || library.id;
    project.activeProtoVersionId = nextLibrary?.defaultVersionId || library.defaultVersionId || "";
  }
  await persistProject(git, project);
  return { action: "archived", library };
}

async function restoreSchema(root, selector) {
  const git = await requireGitWorkspace(root);
  const project = structuredClone(git.project);
  const library = findLibrary(project, selector);
  if (!library) throw new Error(`Schema ${selector} was not found.`);
  ...
  delete library.archivedAt;
  for (const version of library.versions || []) {
    if (version.lifecycle === "archived") {
      version.lifecycle = "active";
      delete version.archivedAt;
    }
  }
  const preferred = findVersion(library, library.defaultVersionId) || (library.versions || [])[0];
  library.defaultVersionId = preferred?.id || "";
  library.updatedAt = now;
  project.activeProtoLibraryId = library.id;
  project.activeProtoVersionId = preferred?.id || "";
  await persistProject(git, project);
  return { action: "restored", library, version: preferred || null };
}

async function archiveRevision(root, selector, revisionSelector) {
  const git = await requireGitWorkspace(root);
  const project = structuredClone(git.project);
  const library = findLibrary(project, selector);
  if (!library) throw new Error(`Schema ${selector} was not found.`);
  const version = findVersion(library, revisionSelector);
  if (!version) throw new Error(`Revision ${revisionSelector} was not found in schema ${library.name}.`);
  if (version.lifecycle === "archived") return { action: "already-archived", library, version };
  const now = new Date().toISOString();
  version.lifecycle = "archived";
  version.archivedAt = now;
  const next = pickNextActiveVersion(library, version.id);
  if (library.defaultVersionId === version.id && next) library.defaultVersionId = next.id;
  library.updatedAt = now;
  if (project.activeProtoLibraryId === library.id && project.activeProtoVersionId === version.id) {
    project.activeProtoVersionId = next?.id || version.id;
  }
  await persistProject(git, project);
  return { action: "archived", library, version, nextVersion: next || null };
}

async function restoreRevision(root, selector, revisionSelector) {
  const git = await requireGitWorkspace(root);
  const project = structuredClone(git.project);
  const library = findLibrary(project, selector);
  if (!library) throw new Error(`Schema ${selector} was not found.`);
  ...
  version.lifecycle = "active";
  delete version.archivedAt;
  const currentDefault = findVersion(library, library.defaultVersionId);
  if (!currentDefault || currentDefault.lifecycle === "archived") library.defaultVersionId = version.id;
  library.updatedAt = now;
  project.activeProtoLibraryId = library.id;
  project.activeProtoVersionId = version.id;
  await persistProject(git, project);
  return { action: "restored", library, version };
}
...
function moveOrMarkRevisionBindings(items, library, sourceVersion, replacementVersion) {
  const replacementMethods = replacementVersion
    ? new Map(discoverProtoMethods(replacementVersion.files || []).map((method) => [methodKey(method), method]))
    : new Map();
  const sourceMethods = new Map(discoverProtoMethods(sourceVersion.files || []).map((method) => [methodKey(method), method]));
  let moved = 0;
  let unresolved = 0;
  const next = (Array.isArray(items) ? items : []).map((item) => {
    if (item?.grpc?.libraryId !== library.id || item?.grpc?.versionId !== sourceVersion.id) return item;
    const methodFullName = String(item.grpc.methodFullName || item.grpcMethodKey || "");
    if (!replacementVersion) {
      unresolved += 1;
      return { ...item, grpc: { ...item.grpc, status: library.versions.length <= 1 ? "library-missing" : "version-missing" } };
    }
    const before = sourceMethods.get(methodFullName);
    const after = replacementMethods.get(methodFullName);
    if (!before || !after || methodSignature(before) !== methodSignature(after)) {
      throw new Error(
        `Request ${item.name || item.title || item.id || methodFullName} cannot move to ${replacementVersion.version}: RPC method is missing or incompatible.`,
      );
    }
    moved += 1;
    return {
      ...item,
      grpcMethodKey: methodFullName,
      grpc: {
        ...item.grpc,
        libraryId: library.id,
        versionId: replacementVersion.id,
        methodFullName,
        schemaChecksum: replacementVersion.checksum || "",
  ...
  return { items: next, moved, unresolved };
}

async function deleteRevision(root, selector, revisionSelector, options = {}) {
  const git = await requireGitWorkspace(root);
  const project = structuredClone(git.project);
  const library = findLibrary(project, selector);
  if (!library) throw new Error(`Schema ${selector} was not found.`);
  const version = findVersion(library, revisionSelector);
  if (!version) throw new Error(`Revision ${revisionSelector} was not found in schema ${library.name}.`);
  const replacement = options.replacement ? findVersion(library, options.replacement) : null;
  if (options.replacement && !replacement) {
    throw new Error(`Replacement revision ${options.replacement} was not found in schema ${library.name}.`);
  }
  if (replacement?.lifecycle === "archived") throw new Error("Replacement revision must be active.");
  if (replacement?.id === version.id) throw new Error("Replacement revision must be different from the revision being deleted.");

  const tabs = moveOrMarkRevisionBindings(project.requestTabs, library, version, replacement);
  let moved = tabs.moved;
  let unresolved = tabs.unresolved;
  project.requestTabs = tabs.items;
  project.collections = (project.collections || []).map((collection) => {
    const result = moveOrMarkRevisionBindings(collection.requests, library, version, replacement);
    moved += result.moved;
    unresolved += result.unresolved;
    return { ...collection, requests: result.items };
  });
  const docs = moveOrMarkRevisionBindings(project.methodDocs, library, version, replacement);
  project.methodDocs = docs.items;
  moved += docs.moved;
  unresolved += docs.unresolved;
  const docResults = moveOrMarkRevisionBindings(project.docResults, library, version, replacement);
  project.docResults = docResults.items;
  moved += docResults.moved;
  unresolved += docResults.unresolved;
  if (project.mockServer?.methodBindings && typeof project.mockServer.methodBindings === "object") {
    const nextBindings = {};
    for (const [key, binding] of Object.entries(project.mockServer.methodBindings)) {
      const result = moveOrMarkRevisionBindings([{ id: key, grpc: binding }], library, version, replacement);
      nextBindings[key] = result.items[0]?.grpc || binding;
      moved += result.moved;
      unresolved += result.unresolved;
    }
    project.mockServer.methodBindings = nextBindings;
  }

  library.versions = (library.versions || []).filter((candidate) => candidate.id !== version.id);
  const removesLibrary = library.versions.length === 0;
  if (removesLibrary) {
    project.protoLibraries = (project.protoLibraries || []).filter((candidate) => candidate.id !== library.id);
  } else {
    const next = replacement || pickNextActiveVersion(library);
    library.defaultVersionId = next?.id || library.versions[0]?.id || "";
    library.updatedAt = new Date().toISOString();
  }

  if (project.activeProtoLibraryId === library.id && project.activeProtoVersionId === version.id) {
    if (removesLibrary) {
      const nextLibrary = (project.protoLibraries || []).find((candidate) => candidate.lifecycle !== "archived") || project.protoLibraries?.[0];
      project.activeProtoLibraryId = nextLibrary?.id || "";
      project.activeProtoVersionId = nextLibrary?.defaultVersionId || "";
    } else {
      project.activeProtoVersionId = library.defaultVersionId;
    }
  }
  await persistProject(git, project);
  return {
    ...
    deleted: { id: version.id, version: version.version },
    replacement: replacement ? { id: replacement.id, version: replacement.version } : null,
    movedRequestCount: moved,
    unresolvedRequestCount: unresolved,
    removedSchema: removesLibrary,
  };
}
