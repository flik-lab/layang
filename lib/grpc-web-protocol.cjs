...
function encodeGrpcWebTextResponseChunks(payloads, status, message = "", metadata = {}) {
  const frames = [
    ...(Array.isArray(payloads) ? payloads : []).map(encodeDataFrame),
    encodeTrailerFrame(status, message, metadata),
  ];
  return frames.map(encodeGrpcWebTextFrame);
}
