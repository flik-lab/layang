...
async function handleCli(parsed, options = {}) {
  ...
  if (parsed.command === "ui") {
    return require("./cli-ui.cjs").handleUiCommand(parsed, { stdout, stderr });
  }
  ...
  if (
    ...
    parsed.command === "schema:archive" ||
    parsed.command === "schema:restore" ||
    parsed.command === "schema:revision-archive" ||
    parsed.command === "schema:revision-restore" ||
    parsed.command === "schema:revision-delete" ||
...
async function handleSchemas(parsed, stdout) {
  ...
  if (parsed.flags.json)
    ...
    for (const schema of schemas) {
      stdout.write(`${schema.name}${schema.lifecycle === "archived" ? " [archived]" : ""}\n`);
      for (const revision of schema.revisions) {
        const state = [revision.isDefault ? "default" : "", revision.lifecycle === "archived" ? "archived" : ""]
          .filter(Boolean)
          .join(", ");
        stdout.write(
          `  - ${revision.label}${state ? ` [${state}]` : ""}: ${revision.fileCount} file(s), ${revision.methods.length} method(s)\n`,
        );
...
async function handleSchemaCommand(parsed, stdout) {
  ...
  } else if (parsed.command === "schema:archive") {
    if (!selector) throw new Error("schema:archive requires --schema.");
    result = await schema.archiveSchema(parsed.workspace || ".", selector);
  } else if (parsed.command === "schema:restore") {
    if (!selector) throw new Error("schema:restore requires --schema.");
    result = await schema.restoreSchema(parsed.workspace || ".", selector);
  } else if (parsed.command === "schema:revision-archive") {
    if (!selector || !parsed.flags.revision)
      throw new Error("schema:revision-archive requires --schema and --revision.");
    result = await schema.archiveRevision(parsed.workspace || ".", selector, parsed.flags.revision);
  } else if (parsed.command === "schema:revision-restore") {
    if (!selector || !parsed.flags.revision)
      throw new Error("schema:revision-restore requires --schema and --revision.");
    result = await schema.restoreRevision(parsed.workspace || ".", selector, parsed.flags.revision);
  } else if (parsed.command === "schema:revision-delete") {
    if (!selector || !parsed.flags.revision)
      throw new Error("schema:revision-delete requires --schema and --revision.");
    if (!parsed.flags.yes) throw new Error("Revision deletion requires --yes.");
    result = await schema.deleteRevision(parsed.workspace || ".", selector, parsed.flags.revision, {
      replacement: parsed.flags.replacement,
    });
  } else {
...
function formatSchemaResult(command, result) {
  ...
  if (command === "schema:revision-delete")
    return `Deleted revision ${result.deleted.version}; ${result.movedRequestCount} moved, ${result.unresolvedRequestCount} unresolved.`;
  if (command === "schema:archive" || command === "schema:restore")
    return `${result.action}: schema ${result.library?.name || "schema"}.`;
  if (command === "schema:revision-archive" || command === "schema:revision-restore")
    return `${result.action}: ${result.library?.name || "schema"} ${result.version?.version || "revision"}.`;
